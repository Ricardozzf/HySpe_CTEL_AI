...
class PhotoMetricDistortion(object):
    ...
    def __call__(self, data):
        ...
        if self.contrast is not None and np.random.randint(2):
            img *= np.random.uniform(self.contrast[0], self.contrast[1])
...
class RandomBorderMask(object):
    """Random mask the images from border.
    ...
    def __init__(self,
                 ...
                 vmask_range=None,
                 ratio=0):
        self.hmask_range = hmask_range
        self.vmask_range = vmask_range
        self.hratio = ratio if hmask_range is not None else 0
        self.vratio = ratio if vmask_range is not None else 0

    def __call__(self, data):
        h, w = data['img'].shape[:2]
        if np.random.uniform(0, 1) < self.hratio:
            sr = int(w * self.hmask_range[0])
            er = int(w * self.hmask_range[1])
            data['img'][:,sr:er+1,:] = 0
        elif np.random.uniform(0, 1) < self.vratio:
            sc = int(h * self.vmask_range[0])
            ec = int(h * self.vmask_range[1])
            data['img'][sc:ec+1,:,:] = 0
        return data
    
class RandomDropChannels(object):
    def __init__(self,
                 num_channel=0.1,
                 shuffle=False,
                 p=0.1):
        ...
        print("load RandomDropChannels")

    def __call__(self, data):
        _,_,c = data['img'].shape
        if np.random.uniform(0, 1) < self.p:
            index = random.sample(range(c), int(c*self.num_channel))
            data['img'][:,:,index] = 0
            if self.shuffle:
                channel_indices = np.random.permutation(c)
                data['img'] = data['img'][:,:,channel_indices]
        return data
    ...
    def __init__(self, pipeline):
        ...
        for k, args in pipeline.items():
            if k in ['RandomFlip', 'RandomRotate', 'RandomCrop',
                     'PatchShuffle', 'RandomErase', 'PhotoMetricDistortion',
                     'CoarseDropout','RandomBorderMask','RandomDropChannels']:
                self.transforms.append(eval(f'{k}')(**args))
            elif k in ['Resize', 'RandomFixSizeCrop', 'Normalize','AlbumAug']:
                continue
            else:
                warnings.warn(f'Augmentation {k} has not yet been implemented')
        # 'Resize and Normalize should be add last'
