...
def encoders(encoder_cfg):
    ...
    if etype in ['mobilenetv1_145', 'mobilenetv1_1201', 'cdcnet', 'cdcnet_pp']:
        ...
        etype = etype.strip("clip_")
        model,preprocess = clip.load(etype, device="cpu")
        return model.visual
    elif 'tf_' in etype:
        etype = etype.strip("tf_")
        ctype = _encoder_cfg.pop("ctype", None)
        config = eval(f'tf.{ctype["cfg_type"]}').from_pretrained(f'{ctype["cfg_path"]}')
        model =  eval(f'tf.{etype}')(config)
        # if _encoder_cfg['pretrained']:
    ...
    if _encoder_cfg['pretrained'] and etype in local_pretrained_dict:
        ...
        try:
            ...
            # support subclass of timm
            model = eval('{}'.format(etype))(**_encoder_cfg)
    return model

def encoders_tf(encoder_cfg):
    config = tf.CLIPVisionConfig("config.json")
    model = tf.CLIPVisionModel(config)
