...
def build_optimizers(cfg, model, **kwargs):
    ...
    optimizer = eval(f'opt.{optimizer_type}')(filter(lambda p:p.requires_grad,model.parameters()),
                                                **cfg_, **kwargs)
    return optimizer
...
def build_schedulers(cfg, optimizer, **kwargs):
    ...
    epoch_iter = cfg_.pop('epoch_iter')
    # if cfg_.pop('T_type', None) == 'Iter':
    #     cfg_['T_max'] *= epoch_iter
