...
from models.fas.swad import AveragedModel, export_swad_model
...
def load_model(load_path, model, optimizer = None, allow_size_mismatch = False):
    ...
    missing_keys, unexpected_keys = model.load_state_dict(loaded_state_dict, strict = not allow_size_mismatch)

    # Load local trained timm vit params into lora
    # wait to do better
    if len(unexpected_keys) != 0:
        loaded_state_dict_new =  OrderedDict()
        for uk in loaded_state_dict.keys():
            if 'qkv' in uk:
                params = loaded_state_dict[uk]
                ic = params.shape[0]
                assert ic % 3 == 0, f"{uk} could not be divided into 3 part!"
                split_size = ic // 3  # 每份的大小
                q = params[:split_size]
                k = params[split_size:2*split_size]
                v = params[2*split_size:]
                
                # module.encoder.blocks.11.attn.qkv.weight
                # module.encoder.blocks.10.attn.W_v.linear.weight

                loaded_state_dict_new[uk.replace("qkv","W_q.linear")] = q
                loaded_state_dict_new[uk.replace("qkv","W_k")] = k
                loaded_state_dict_new[uk.replace("qkv","W_v.linear")] = v
                
            else:
                loaded_state_dict_new[uk] = loaded_state_dict[uk]
        
        missing_keys, unexpected_keys = model.load_state_dict(loaded_state_dict_new, strict = not allow_size_mismatch)
...
class Runner(object):
    ...
    def _init_model(self, resume_from=None, load_from=None, pretrain_from=None):
        ...
        if load_from is not None:
            ...
            self._score = checkpoint.get('score', 0)
            loaded_state_dict = {k.replace("module.module.", "module."):v for k,v in checkpoint['state_dict'].items()}
    ...
    def _train_epoch(self, dataloader):
        ...
        for i, data in enumerate(dataloader):
            ...
            if self._iter == self.check_cfg.iters:
                break

    @torch.no_grad()
    def val(self, showtitle='Val Loss'):
        """val method"""
        feats = list()
        preds = list()
        labels = list()
        ...
        for data in tqdm(self.val_dataloader):
            ...
            loss_sum += output['loss'].detach().cpu().numpy()
            if 'feat' in output:
        ...
        score, eval_dict = self.metric(preds, labels, paths=paths, thr=self.eval_thr)
        self.logger.info(f'{showtitle}: {loss_sum / len(labels)}')
        if len(feats) > 0:
        ...
        return score, loss_sum / len(labels)
    ...
    @torch.no_grad()
    def infer(self, dataloader, filename=None, ann_nums=None, log_info=True):
        """infer method"""
        preds = list()
        paths = list()
        self.model.eval()
        ...
        preds = np.concatenate(preds)
        paths = np.array(paths)
        result = np.hstack((paths[:, np.newaxis], 1-preds[:, np.newaxis]))
        dump(result, os.path.join(self.work_dir, "score.txt"))
        return None
...
class RunnerSwad(Runner):
    def __init__(self, model, logger, work_dir, log_cfg=None, eval_cfg=None, optim_cfg=None, sched_cfg=None, check_cfg=None, freeze_cfg=None):
        super().__init__(model, logger, work_dir, log_cfg, eval_cfg, optim_cfg, sched_cfg, check_cfg, freeze_cfg)
    ...
    def train(self, dataloader, cfg):
        ...
        self._total_epoch = cfg.total_epochs
        self._total_iter = self._get_total_iter(dataloader)
        self._train_swad = cfg.get('train_swad', None)
        ...
        self.model.train()
        start_epoch = self._total_epoch if self._train_swad.get('only_swad') else self._epoch 
        for epoch in range(start_epoch, self._total_epoch):
